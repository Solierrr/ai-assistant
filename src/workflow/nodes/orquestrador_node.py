...
from src.prompts.system_prompts import PROMPT_ORQUESTRADOR

def no_orquestrador(estado: Estado) -> dict:

    prompt_sistema = PROMPT_ORQUESTRADOR.format(rota=estado["rota"])
    
    mensagens_com_contexto = [SystemMessage(content=prompt_sistema)] + estado["messages"]
    saida = llm_gemini().with_fallbacks([llm_groq()]).invoke(mensagens_com_contexto).content
