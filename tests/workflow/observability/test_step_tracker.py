...
def test_step_tracker_detecta_node_sozinho_via_metadata(monkeypatch):
    ...
    async def cenario():
        ...
        await tracker.on_llm_end(_resposta_llm(), run_id="run-x")
        await tracker.flush()
...
def test_step_tracker_usa_desconhecido_sem_metadata(monkeypatch):
    ...
    async def cenario():
        await tracker.on_llm_start({}, ["prompt"], run_id="run-y")
        await tracker.on_llm_end(_resposta_llm(), run_id="run-y")
        await tracker.flush()
...
def test_step_tracker_inclui_conversation_id(monkeypatch):
    ...
    async def cenario():
        ...
        await tracker.on_llm_end(_resposta_llm(), run_id="run-z")
        await tracker.flush()
...
def test_step_tracker_registra_erro_sem_derrubar_fluxo(monkeypatch):
    ...
    async def cenario():
        ...
        await tracker.on_llm_error(RuntimeError("falha no groq"), run_id="run-e")
        await tracker.flush()
...
def test_step_tracker_envia_contrato_completo_do_api_messenger(monkeypatch):
    enviar = AsyncMock()
    monkeypatch.setattr(step_tracker, "enviar_observabilidade", enviar)
    tracker = step_tracker.StepTracker(conversation_id="conv-1", environment="LOCAL")

    async def cenario():
        await tracker.on_llm_start({}, ["prompt"], run_id="run-1")
        await tracker.on_llm_end(
            _resposta_llm(tokens_in=10, tokens_out=5), run_id="run-1"
        )
        await tracker.flush()
